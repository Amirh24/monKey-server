...
class SVGAppender(object):
    ...
    def append(self, element, new_color=None):
        ...
        try:
            ...
            class_elements = element.root.findall('.')
            for c in class_elements:
                self.modifygroupvalues(c)
    ...
    def modifystylevalues(self,css_string, new_color):
        ...
        for rule in sheet:
            # If the accessory is colorable, change color
            if new_color is not None:
                if rule.style.getPropertyValue('fill') == self.to_color_sign:
                    rule.style.setProperty('fill', value=new_color, replace=True)
    ...
    def monkeycolorchange(self, fur_color, eye_color, background_color=None):

        original_fur_color = '#7f6145'
        original_eye_color = '#313cc4'

        css_string = self.style_element.text
        sheet = cssutils.parseString(css_string)
        # Add a background to the monKey image if needed
        if background_color is not None:
            sheet.cssRules[0].style.setProperty('fill', value=background_color, replace=True)
        for rule in sheet:
            if rule.style.getPropertyValue('fill') == original_fur_color:
                rule.style.setProperty('fill',  value=fur_color, replace=True)
            if rule.style.getPropertyValue('fill') == original_eye_color:
                rule.style.setProperty('fill',  value=eye_color, replace=True)

            css_without_newline_chars = str(sheet.cssText)[2:].replace('\\n', '')
            translator = str.maketrans('', '', ' \t\r')
            self.style_element.text = css_without_newline_chars.translate(translator)
